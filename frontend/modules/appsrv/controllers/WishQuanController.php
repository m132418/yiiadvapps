 ...
 */
class WishQuanController extends AppSrvBase2Controller
{
...
//         var_dump($rows) ;exit() ;
      $data = [];$ddone=[];
        ...
        foreach ($rows as $key => $value) {
         ...
         $ddone["nickname"] = is_null($d_profile["nickname"]) ? "":$d_profile["nickname"] ;
         $ddone["signature"] = is_null($d_profile["signature"]) ? "":$d_profile["signature"] ;
//          var_dump($d_profile) ;
//             var_dump($value) ;
//             var_dump($ddone) ;
//             exit() ;
...
//         $rows[$key]["nickname"] = $nickname["nickname"] ;
        $value["nickname"] =is_null($nickname["nickname"])? "":$nickname["nickname"]  ;
...
//                   
                    ...
                    foreach ($c_rows as $key2 => $re_comment) {
                        
                        $c_rows[$key2]["re-nickname"] = $value["nickname"] ;        
                        $c_rows[$key2]["re-uid"] = $value["id"] ;
                    }
                    
                    $coments[$key]["re-comments"] = $c_rows ;
                    
                    $ddone["coments"] = array_merge(  $ddone["coments"],$c_rows) ;
...
//          $data[$key] =  $ddone    ;
         array_push($data,$ddone) ;
         unset($ddone) ;
        }
        
        parent::rtn_json(['cd'=>"1","data"=> (array)$data]) ;
...
//         var_dump($data["dd_id"]) ;
        ...
        "SELECT wish.id AS wish_id,                      ".
            ...
            "where deposit.uid = 0 and   dongdan.id =" . $data["did"]
                ;
...
//                       'user_profile.portrait',
        ...
        parent::rtn_json(['cd'=>"1","comments"=>$data]) ;
    }
